...
export async function enviarFacturaPagada(datosPago) {
  try {
    console.log('📧 Iniciando envío de confirmación de pago...');
    console.log('📋 Datos del pago:', JSON.stringify(datosPago, null, 2));
    ...
    const fechaPagoFormato = fechaPagoObj.toLocaleDateString('es-ES', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="invoice-header">
            <div class="invoice-number">
              Recibo Nº <strong>#${String(pago_id).padStart(6, '0')}</strong>
            </div>
            <div style="text-align: right; color: #6b7280; font-size: 14px;">
              <div><strong>Fecha de Pago:</strong></div>
              <div style="color: #10b981; font-weight: 600;">${fechaPagoFormato}</div>
              <div style="margin-top: 10px;"><strong>Método de Pago:</strong></div>
              <div>${metodoNombre}</div>
            </div>
          </div>
          ...
          <table class="details-table">
            ...
            <tbody>
              ${tipo_pago === 'mantenimiento' || tipo_pago === 'reservas' ? `
                ${tipo_pago === 'mantenimiento' && montoMantenimiento > 0 ? `
                <tr>
                  <td>
                    <div class="item-name">🏠 Mantenimiento Mensual</div>
                    <div class="item-description">${descripcion.split(' + Reservas')[0]}</div>
                  </td>
                  <td class="amount">$${montoMantenimiento.toFixed(2)}</td>
                </tr>
                ` : ''}
                ...
                <tr>
                  <td colspan="2" style="padding: 0;">
                    <div style="padding: 15px 12px;">
                      <div class="item-name" style="margin-bottom: 10px;">🏊 Reservas de Áreas Comunes Confirmadas</div>
              ...
              ` : `
                <tr>
                  <td>
                    <div class="item-name">${descripcion}</div>
                  </td>
                  <td class="amount">$${parseFloat(monto_total).toFixed(2)}</td>
                </tr>
              `}
          ...
          <div class="payment-info">
            ...
            ${tieneReservas ? `Sus ${reservas.length} reserva${reservas.length > 1 ? 's' : ''} de áreas comunes han sido confirmadas.` : ''}
        ...
        <div class="footer">
          ...
          <p style="margin: 5px 0 0 0;">Este es un comprobante de pago automático, por favor no responder a este correo.</p>
        </div>
      </div>
    </body>
    </html>
    ...
    sendSmtpEmail.subject = `✅ Pago Confirmado #${String(pago_id).padStart(6, '0')} - Habitech`;
    ...
    console.log('📤 Enviando confirmación de pago con Brevo SDK...');
    ...
    console.error('❌ Error al enviar confirmación de pago:', error.message);
    console.error('❌ Stack:', error.stack);
    // No lanzar error para que no falle la actualización del pago
  }
}
