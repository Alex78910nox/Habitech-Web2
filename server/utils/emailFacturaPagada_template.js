...
import fs from 'fs';
import { generarFacturaPendientePDF } from './generarFacturaPendientePDF.js';
...
export async function enviarFacturaPago(datosPago) {
  try {
    console.log('📧 Iniciando envío de factura de pago...');
    ...
    const { 
      ...
      fecha_vencimiento,
      descripcion,
      reservas = [] // Array de reservas incluidas
    } = datosPago;
    ...
    <head>
      ...
      <style>
        ...
        .header {
          background: linear-gradient(135deg, #1a2744 0%, #6366f1 100%);
          color: white;
          padding: 30px;
          text-align: center;
        }
    ...
    console.log('� Generando PDF de la factura pendiente...');
    
    // Generar PDF de la factura
    let pdfPath;
    try {
      pdfPath = await generarFacturaPendientePDF(datosPago);
      ...
      sendSmtpEmail.attachment = [
        {
          content: pdfBase64,
          name: `Factura_${String(pago_id).padStart(6, '0')}.pdf`
        }
      ];

      console.log('📎 PDF adjuntado al email');
    } catch (pdfError) {
      console.error('⚠️ Error al generar PDF, enviando email sin adjunto:', pdfError);
    }

    console.log('�📤 Enviando factura de pago con Brevo SDK...');
    ...
    console.log('✅ Factura de pago enviada exitosamente!');
    ...
    if (pdfPath) {
      try {
        ...
        console.log('🗑️ Archivo temporal PDF eliminado');
      } catch (deleteError) {
        console.error('⚠️ Error al eliminar archivo temporal:', deleteError);
      }
    }
    ...
    console.error('❌ Error al enviar factura de pago:', error.message);
